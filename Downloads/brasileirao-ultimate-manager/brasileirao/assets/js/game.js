...
function gameTick() {
  matches.forEach((m, idx) => {
    ...
    if (Math.random() < 0.00068) {
      ...
      const team = isHome ? m.h : m.a;
      isHome ? m.sh++ : m.sa++;
      const scorer = regScorer(team);
      addLog(idx, mStr, 'g', pickNarr('goalMoment', {P:scorer, T:team}));
      if (idx===activeMIdx) {
        showGoalCeleb(scorer, team);
        ball.x=.5; ball.y=.5; ball.vx=0; ball.vy=0; ball.spin=0;
        allPlayers.forEach(p => { p.x=p.bx; p.y=p.by; p.vx=0; p.vy=0; });
      }
    }
    ...
    if (Math.random() < 0.003) {
      ...
      const pName  = pickPlayer(team);
      const gkName = pickGK(opp);

      if      (r > 0.97) { addLog(idx,mStr,'r', `🟥 EXPULSO! <b>${pName}</b> (${team}) recebe cartão vermelho!`); if(idx===activeMIdx){const pl=allPlayers.find(p=>p.name===pName&&p.team===team);if(pl)pl.expulso=true;} }
      else if (r > 0.93) { addLog(idx,mStr,'pen', pickNarr('penalty',{P:pName,T:team})); setTimeout(()=>{ if(Math.random()<0.78){isHome?m.sh++:m.sa++;const sc=regScorer(team);addLog(idx,mStr,'g',pickNarr('penaltyScored',{P:sc,T:team}));if(idx===activeMIdx)showGoalCeleb(sc,team);}else{addLog(idx,mStr,'',pickNarr('penaltyMissed',{P:pName}));}},800); }
      else if (r > 0.88) { addLog(idx,mStr,'y', `🟨 Amarelo para <b>${pName}</b> (${team}).`); }
      else if (r > 0.82) { addLog(idx,mStr,'foul', pickNarr('freeKick',{P:pName,T:team})); }
      else if (r > 0.76) { addLog(idx,mStr,'',    pickNarr('corner',{P:pName,T:team})); }
      else if (r > 0.70) { addLog(idx,mStr,'',    pickNarr('save',{P:pName,GK:gkName,T:team})); }
      else if (r > 0.63) { addLog(idx,mStr,'',    pickNarr('shot',{P:pName,T:team})); }
      else if (r > 0.56) { addLog(idx,mStr,'',    pickNarr('dribble',{P:pName,T:team})); }
      else if (r > 0.50) { addLog(idx,mStr,'foul', pickNarr('foul',{P:pName})); }
      else if (r > 0.44) { addLog(idx,mStr,'',    pickNarr('offside',{P:pName,T:team})); }
      else               { addLog(idx,mStr,'',    pickNarr('possession',{P:pName,T:team})); }
    }

    /* Comentário de posse adicional */
    if (Math.random() < 0.002) {
      const t2 = Math.random()>0.5 ? m.h : m.a;
      addLog(idx, mStr, '', pickNarr('possession', {P:pickPlayer(t2), T:t2}));
    }
  });
...
function updateUI() {
  renderRoundList(); renderBetsLive();
...
function regScorer(team) {
  ...
  const sc = pool[Math.floor(Math.random()*pool.length)] || t.p[1];
